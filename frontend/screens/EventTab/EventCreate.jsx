    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <CustomDateTimePicker
          title={"Bắt đầu"}
          onPressTime={() => showTimepicker("start")}
          onPressDate={() => showDatepicker("start")}
          time={date.start}
          date={date.start}
        />
        <CustomDateTimePicker
          title={"Kết thúc"}
          onPressTime={() => showTimepicker("end")}
          onPressDate={() => showDatepicker("end")}
          time={date.end}
          date={date.end}
        />
      ...
      <SlideModal
        setModalVisible={setModalImageVisible}
        modalVisible={modalImageVisible}
      >
        <View>
          <Text style={styles.title}>
            Các hình ảnh sự kiện ({imageList.length})
          </Text>
          <FlatList
            data={imageList}
            ...
            renderItem={({ item, index }) => (
              <ImagePost
                uri={item}
                index={index}
                onPressDelete={handleImageDelete}
              />
            )}
            ListFooterComponent={
              <CustomButton
      ...
      {show !== 0 && (
        <DateTimePicker
          ...
          display="spinner"
          value={show === 1 ? date.start : date.end}
          mode={mode}
          is24Hour={true}
          onChange={onChange}
        />
